...
void push(ll nd, ll l, ll r) {
    if (lazy[nd]) {
        sgtree[nd] = max(sgtree[nd] , lazy[nd] ) ; 
        if (l != r) { 
...
ll query(ll nd, ll l, ll r, ll ql, ll qr) {
    ...
    ll md = (l + r) / 2;
    return max( query(2 * nd, l, md, ql, qr) , query(2 * nd + 1, md + 1, r, ql, qr) );
}
...
void range_update(ll nd, ll l, ll r, ll ql, ll qr, ll val) {
    ...
    sgtree[nd] = max( sgtree[2 * nd] , sgtree[2 * nd + 1] );
}
...
void build(vector<ll> &a, ll &n) {
    ...
    for (ll i = n - 1; i >= 1; i--) {
        sgtree[i] = max( sgtree[i * 2] , sgtree[i * 2 + 1] );
    }
}
int main()
{
